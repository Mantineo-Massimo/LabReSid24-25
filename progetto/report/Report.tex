...
\begin{itemize}
    \item Il dispatcher agisce da \textbf{Produttore}. Creato un job, incrementa il conteggio del semaforo con \texttt{sem\_post(\&pool->queue\_sem)}.
    \item I \textit{Worker} sono i \textbf{Consumatori}. All'avvio chiamano \texttt{sem\_wait(\&pool->queue\_sem)}. A coda vuota, il kernel blocca il thread in stato di \textit{sleep}, evitando di consumare cicli di CPU (\textit{busy-waiting}).
\end{itemize}
