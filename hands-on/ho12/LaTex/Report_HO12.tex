...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        % Interactions
        \draw[arrow] (prod) -- node[above, font=\tiny] {put\_item()} (buf);
        \draw[arrow] (buf) -- node[above, font=\tiny] {get\_item()} (cons);
        \draw[arrow, <->, red!70] (sem) -- (buf);

        % Labels
        \node[below=0.1cm of prod, font=\tiny] {sem\_wait(empty\_slots)};
        \node[below=0.1cm of cons, font=\tiny] {sem\_wait(filled\_slots)};
    \end{tikzpicture}
