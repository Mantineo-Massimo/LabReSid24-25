...
\begin{itemize}
    \item \textbf{SO\_REUSEADDR}: Modificare le opzioni originarie del socket affinchè il sistema permetta il collegamento di \textit{bound} sulla medesima porta e lo stesso IP a più processi.
    \item \textbf{Binding su INADDR\_ANY}: Necessario per catturare i pacchetti dalla scheda di rete che transitano sul gruppo al quale ci sottoscriviamo.
    \item \textbf{IP\_ADD\_MEMBERSHIP}: Direttiva tramite cui il socket comunica l'intenzione di aggregarsi ad uno specifico indirizzo IP multicast.
\end{itemize}
...
int reuse = 1;
if (setsockopt(sockfd, SOL\_SOCKET, SO\_REUSEADDR, &reuse, sizeof(reuse)) < 0) {
    perror("SO\_REUSEADDR failed");
    exit(EXIT\_FAILURE);
}
\end{ccode}
...
// Bind
local\_addr.sin\_family = AF\_INET;
local\_addr.sin\_port = htons(PORT);
local\_addr.sin\_addr.s\_addr = INADDR\_ANY;
bind(sockfd, (struct sockaddr *)&local\_addr, sizeof(local\_addr));

// Iscrizione Multicast
struct ip\_mreq multicast\_req;
multicast\_req.imr\_multiaddr.s\_addr = inet\_addr("239.0.0.1");
multicast\_req.imr\_interface.s\_addr = INADDR\_ANY;

setsockopt(sockfd, IPPROTO\_IP, IP\_ADD\_MEMBERSHIP, &multicast\_req, sizeof(multicast\_req));
\end{ccode}
...
while (1) {
    int n = recvfrom(sockfd, buffer, MAX\_BUFFER - 1, 0, 
                     (struct sockaddr *)&sender\_addr, &sender\_len);
    // ... logic per stampa e gestione terminazione ...
}

// 6: Dropout in dismissione
setsockopt(sockfd, IPPROTO\_IP, IP\_DROP\_MEMBERSHIP, &multicast\_req, sizeof(multicast\_req));
\end{ccode}
...
$ gcc multicast\_server.c -o server
$ gcc multicast\_client.c -o client
\end{terminal}
