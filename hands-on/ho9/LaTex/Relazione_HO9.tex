...
\title{\vspace{-2cm}Relazione: Tabella dei Simboli e Compilazione Modulare\\
\large (Hands-On 9)}
...
È stata creata un'architettura multi-file (\texttt{main.c}, \texttt{ex1_symbols.c}) per osservare il comportamento dei simboli \texttt{U} (Undefined) e \texttt{T} (Text).
\begin{terminal}[Analisi Tabella Simboli]
$ nm ex1_symbols.o
0000000000000000 D var_globale   <-- Data (D)
                 U printf        <-- Indefinito (U)
0000000000000000 T funzione_test <-- Text (T)
\end{terminal}
