...
\title{\vspace{-2cm}Report: Implementazione ICMP con Socket Raw\\
\large (Hands-On 5)}
...
unsigned short compute\_checksum(void *addr, int len) {
...
struct iphdr *ip\_hdr = (struct iphdr *)recv\_buffer;
// L'header ICMP inizia dopo (IHL * 4) byte
struct icmphdr *icmp\_reply = (struct icmphdr *)(recv\_buffer + (ip\_hdr->ihl * 4));
\end{ccode}
...
$ gcc ICMP\_raw.c -o icmp\_ping
$ sudo ./icmp\_ping 8.8.8.8
\end{terminal}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{ho5\_icmp\_request.png}
    \caption{Cattura pacchetti ICMP su Wireshark.}
    \label{fig:icmp\_capture}
\end{figure}
