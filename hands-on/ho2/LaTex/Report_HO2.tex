...
\title{\vspace{-2cm}Report: Server TCP Bloccante e Non-Bloccante\\
\large (Hands-On 2)}
...
\begin{itemize}
    ...
    \item \textbf{I/O Non-Bloccante}: Tramite la syscall \texttt{fcntl(O\_NONBLOCK)}, il socket viene istruito a non sospendere il processo; se i dati non sono pronti, la funzione ritorna immediatamente l'errore \texttt{EAGAIN}.
...
int flags = fcntl(master\_socket, F\_GETFL, 0);
fcntl(master\_socket, F\_SETFL, flags | O\_NONBLOCK);
activity = select(max\_sd + 1, &readfds, NULL, NULL, NULL);
\end{ccode}
Questa architettura permette di mantenere stati di validazione indipendenti per ogni client tramite una struttura \texttt{client\_state}.


\subsection{Istruzioni di Esecuzione}
La compilazione dei modelli bloccanti e non-bloccanti avviene tramite GCC:
\begin{terminal}[GCC Build]
$ gcc server\_tcp\_block.c -o s\_block
$ gcc server\_tcp\_nonblock.c -o s\_nonblock
$ gcc client\_tcp.c -o client
\end{terminal}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{\detokenize{ho2_wireshark_nonblocking.png}}
    \caption{Monitoraggio socket non-bloccanti su Wireshark.}
    \label{fig:wireshark_nonblocking}
\end{figure}
...
Received: 0 from socket 4 -> ACK\_FIB\_OK
Received: 1 from socket 5 -> ACK\_FIB\_OK (Gestione Parallela)
\end{terminal}
