...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        node distance=2cm,
        agent/.style={circle, draw=blue!70, fill=blue!5, thick, minimum size=1.2cm, align=center, font=\small\bfseries},
        fifo/.style={rectangle, draw=orange!70, fill=orange!5, dashed, font=\tiny\ttfamily, align=center},
        proc/.style={draw=blue!70, fill=blue!5, thick, circle, minimum size=1.5cm, align=center, font=\small\sffamily},
        arrow/.style={-Stealth, thick, blue!70}
    ]
        % Nodes
        \node[agent] (a0) {Agente 0\\(Init)};
        \node[agent, below left=of a0] (a1) {Agente 1};
        \node[agent, below right=of a0] (a2) {Agente 2};
        \node[agent, below=3.5cm of a0] (a3) {Agente 3};

        % Edges (FIFOs)
        \draw[arrow] (a0) -- node[above left, font=\tiny, color=black] {/tmp/fifo\_0\_1} (a1);
        \draw[arrow] (a0) -- node[above right, font=\tiny, color=black] {/tmp/fifo\_0\_2} (a2);
        \draw[arrow] (a1) -- node[below left, font=\tiny, color=black] {/tmp/fifo\_1\_3} (a3);
        \draw[arrow] (a2) -- node[below right, font=\tiny, color=black] {/tmp/fifo\_2\_3} (a3);
        
        % Annotations
        \node[fifo, right=1.5cm of a2, align=center] {Inter-Process\\Communication\\via Named Pipes};
    \end{tikzpicture}
    \caption{Topologia distribuita a diamante per la simulazione dell'algoritmo di Flooding.}
    \label{fig:flooding_topology}
\end{figure}
...
\begin{terminal}[Simulazione Algoritmo di Flooding]
...
\begin{itemize}
    \item \textbf{High-Performance Ring Election}: Evoluzione del grafo a diamante in una struttura a anello per testare gli algoritmi LCR e FloodMax.
    \item \textbf{Signal-Driven I/O}: Sostituzione della \texttt{select()} con la gestione asincrona tramite segnali real-time (\texttt{SIGRT}).
\end{itemize}
\end{document}