...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        node distance=2cm,
        agent/.style={circle, draw=blue!70, fill=blue!5, thick, minimum size=1.2cm, font=\small\bfseries},
        fifo/.style={rectangle, draw=orange!70, fill=orange!5, dashed, font=\tiny\ttfamily},
        ...
        % Annotations
        \node[fifo, right=0.5cm of a2, align=center] {Inter-Process\\Communication\\via Named Pipes};
    \end{tikzpicture}
    \caption{Topologia distribuita a diamante per la simulazione dell'algoritmo di Flooding.}
    \label{fig:flooding\_topology}
\end{figure}
...
for(int i = 0; i < MAX\_AGENTS; i++) {
    for(int j = 0; j < num\_neighbors[i]; j++) {
        char fname[64];
        sprintf(fname, "/tmp/fifo\_%d\_%d", i, neighbors[i][j]);
        mkfifo(fname, 0666);
    }
}
\end{ccode}
...
void stf(int id, int* state, int sender, const char* info) {
    if (*state == 0) { // PASSIVE TO DONE
        *state = 1; 
        for(int i = 0; i < num\_neighbors[id]; i++) {
...
[AGENTE 1] RICEZIONE da Nodo 0: 'ALLERTA\_METEO'. TRASIZIONE a DONE e Propagazione.
    -> [AGENTE 1] Propaga messaggio al Nodo 3
[AGENTE 2] RICEZIONE da Nodo 0: 'ALLERTA\_METEO'. TRASIZIONE a DONE e Propagazione.
    -> [AGENTE 2] Propaga messaggio al Nodo 3
[AGENTE 3] RICEZIONE da Nodo 1: 'ALLERTA\_METEO'. TRASIZIONE a DONE e Propagazione.
