...
\title{\vspace{-2cm}Relazione: Meccanismi di Sincronizzazione\\
\large (Hands-On 11)}
...
void* thread_function(void* arg) {
    ...
    for (int i = 0; i < MAX_VALUE; i++) {
        shared_counter++; // <--- Non atomico: Read, Increment, Write
    }
...
for (int i = 0; i < MAX_VALUE; i++) {
    ...
    shared_counter++; // <--- Protetto al 100% da accesso multi-thread
...
sem_wait(&max_concurrency_sem); // Thread attende il token
// ... Critical Section accessibile per due thread contemporanei ...
sleep(2); // Lavoro pesante simulato
sem_post(&max_concurrency_sem); // Ritiro e rilascio
\end{ccode}
...
massimo@PORTATILE-Linux:~/Scrivania/LabReSid24-25/hands-on/ho11$ ./ex1_no_sync
Thread 9: shared_counter = 76935
Final value of shared_counter: 76935 (Expected: 100000)
\end{terminal}
...
massimo@PORTATILE-Linux:~/Scrivania/LabReSid24-25/hands-on/ho11$ ./ex2_sync
Thread 4: shared_counter = 100000
Final value of shared_counter: 100000 (Expected: 100000)
\end{terminal}
...
massimo@PORTATILE-Linux:~/Scrivania/LabReSid24-25/hands-on/ho11$ ./ex3_semaphore
