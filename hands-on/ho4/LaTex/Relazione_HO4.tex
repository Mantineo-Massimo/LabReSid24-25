...
\title{\vspace{-2cm}Relazione: Server TCP ad Alte Prestazioni con \texttt{epoll()}\\
\large (Hands-On 4)}
...
Il server rimane in attesa bloccante su \texttt{epoll_wait}, che restituisce solo i descrittori pronti:
\begin{ccode}[Snippet Loop degli Eventi]
int nfds = epoll_wait(epoll_fd, events, MAX_EVENTS, -1);
for (int n = 0; n < nfds; ++n) {
    if (events[n].data.fd == listen_fd) {
        // Accettazione e aggiunta nuovo client con EPOLL_CTL_ADD
    } else {
        // Lettura dati e invio ACK_EPOLL_OK
    }
}
\end{ccode}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{ho4/ho4_wireshark_multi.png}
    \caption{Cattura epoll multi-client su Wireshark.}
    \label{fig:wireshark_epoll_multi}
\end{figure}
...
L'ispezione della cattura (Figura~\ref{fig:wireshark_epoll_multi}) evidenzia che il server \texttt{epoll} risponde istantaneamente a ogni pacchetto \texttt{PSH} ricevuto. La stabilità della connessione è garantita dalla gestione asincrona degli acknowledgment a livello applicativo.

\subsection{Performance Benchmark}
I grafici prestazionali (Figura 4) mostrano che per carichi piccoli la differenza con la \texttt{select} è trascurabile, ma la stabilità del tempo di risposta conferma l'efficienza del meccanismo di notifica kernel-based.
...
\begin{itemize}
    \item \textbf{io_uring}: Esplorazione del nuovo sottosistema Linux \texttt{io_uring} per eliminare completamente le syscall bloccanti tramite code di completamento asincrone.
