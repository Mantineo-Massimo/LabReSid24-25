...
\begin{itemize}
    \item \textbf{I/O Multiplexing Efficiente}: A differenza di \texttt{select}, il tempo di risposta di \texttt{epoll} non scala linearmente con il numero di descrittori monitorati.
    \item \textbf{Meccanismo Event-driven}: Il kernel notifica l'applicazione solo quando si verifica un evento reale, eliminando la necessità di iterare su migliaia di FD inattivi.
    \item \textbf{Scalabilità Cloud-native}: Ideale per gestire migliaia di connessioni concorrenti (C10K problem) con un impiego minimo di risorse.
\end{itemize}
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        \node[fd, fill=yellow!30] (fd_act) at (1.5,0) {fd 8};
        \node[right=0.1cm of fd_act, font=\tiny, color=red] {EVENT!};
        ...
        \draw[arrows={-Stealth}, thick, red] (fd_act) -- (ready);

        % User Space
        \node[box, below=1.5cm of ready] (user) {epoll\_wait()};
        \draw[arrows={Stealth-Stealth}, thick, green!50!black] (ready) -- node[right, font=\tiny] {O(k) Return} (user);
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{ho4_wireshark_multi.png}
